...
External gate evidence is tracked under #1115. `docs/intent/generative-ui-plugin/external-gate-evidence.json` is the pending source file for approved OpenAI/Codex plugin IDs, the renderer app ID, ChatGPT developer-mode transcript evidence, authenticated Claude Code smoke prompts for both split Claude plugins, human/policy review, and all four marketplace submission receipts. The checker reports pending fields and recorder command hints without regenerating packages. The generated `docs/intent/generative-ui-plugin/submission-artifacts/current/external-gates.md` runbook turns the same evidence into a submitter checklist. The recorder CLI updates the tracked source file without hand-editing the schema. `validate-external-gate-recorder.mjs` exercises every recorder command against a temporary evidence file and proves all gates can transition to `proved` without mutating the tracked pending source file. CI validates the template, tracked evidence source, recorder flow, and public runbook link. The artifact generator copies and validates that file into the public artifact bundle, and the submission audit promotes external gates only when the required fields are complete.
