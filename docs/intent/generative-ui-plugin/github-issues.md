...
| [#1115](https://github.com/repetere/jsonx/issues/1115) | Store submission | Split OpenAI/Codex and Claude Code submission materials, generated review packages, portal submission packets, ChatGPT app submission JSON, app IDs, screenshots, privacy links, test prompts, and external gate evidence. |
...
Submission artifact coverage is tracked under #1115. Current generated artifacts live under `docs/intent/generative-ui-plugin/submission-artifacts/current/`, including split core and generative UI plugin review zips, store listing drafts, portal submission packets, screenshot evidence, golden-prompt tool-call evidence, renderer motion evidence, browser demo mode evidence, hosted MCP transcript evidence, skill installer evidence, isolated Codex marketplace install evidence, Claude Code validation evidence, OpenCode skill discovery evidence, GitHub issue evidence, external gate access evidence, external gate evidence, an external gate runbook, a requirement audit, a copied ChatGPT submission JSON, a copied Codex marketplace file, hashes, and npm package-boundary evidence. Regenerate with `node docs/intent/generative-ui-plugin/scripts/prepare-submission-artifacts.mjs`.

GitHub issue evidence is tracked under #1115. `docs/intent/generative-ui-plugin/scripts/check-github-issue-tracking.mjs` fetches #1110 through #1117 from the GitHub Issues API and validates that every workstream issue is open, linked from this file, and labeled `enhancement` and `codex`. The generated artifact is `docs/intent/generative-ui-plugin/submission-artifacts/current/github-issue-evidence.json`.

External gate access evidence is tracked under #1115. `docs/intent/generative-ui-plugin/scripts/check-external-gate-access.mjs` probes hosted MCP health, marketplace portal reachability, and local Claude Code auth state without printing credential values. The generated artifact is `docs/intent/generative-ui-plugin/submission-artifacts/current/external-gate-access.json`.

External gate evidence is tracked under #1115. `docs/intent/generative-ui-plugin/external-gate-evidence.json` is the pending source file for approved OpenAI/Codex plugin IDs, the renderer app ID, ChatGPT developer-mode transcript evidence, authenticated Claude Code smoke prompts for both split Claude plugins, human/policy review, and all four marketplace submission receipts. The checker reports pending fields and recorder command hints without regenerating packages. The generated `docs/intent/generative-ui-plugin/submission-artifacts/current/external-gates.md` runbook turns the same evidence into a submitter checklist and lists `https://platform.openai.com/plugins`, `https://platform.claude.com/plugins/submit`, and the Claude Team/Enterprise organization form where applicable. The recorder CLI updates the tracked source file without hand-editing the schema. `validate-external-gate-recorder.mjs` exercises every recorder command against a temporary evidence file and proves all gates can transition to `proved` without mutating the tracked pending source file. CI validates the template, tracked evidence source, recorder flow, public runbook link, and submission portal metadata. The artifact generator copies and validates that file into the public artifact bundle, and the submission audit promotes external gates only when the required fields are complete.

Goal completion audit coverage is tracked under #1115. `docs/intent/generative-ui-plugin/scripts/audit-generative-ui-goal.mjs` checks the full objective against current repo files and generated evidence, reports implementation regressions as failures, and keeps app IDs, ChatGPT developer-mode transcripts, authenticated Claude smoke prompts, policy review, and marketplace receipts as explicit external-gated items until recorded.

CI coverage is tracked under #1115. `.github/workflows/generative-ui-plugin.yml` validates plugin metadata, renderer checks, fixtures, mirrored skill docs, the public generative UI page review-kit links, the goal completion audit, the external gate evidence template, the tracked external gate evidence source, the external gate recorder flow, generated submission packages, and npm package exclusions on push, pull request, and manual dispatch.
