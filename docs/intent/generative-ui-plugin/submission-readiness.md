...
- Inspect external gate evidence with `node docs/intent/generative-ui-plugin/scripts/check-external-gate-evidence.mjs`. Use `--init --force` only when resetting `docs/intent/generative-ui-plugin/external-gate-evidence.json` from the template.
